...
<?php
	...
	public function form_fields( $fields, $formid, $form ) {
		...
		if ( is_user_logged_in() && mailster_option( 'coolcaptcha_loggedin' ) ) { return $fields;
		}

		if ( ! in_array( $formid, mailster_option( 'coolcaptcha_forms', array() ) ) ) { return $fields;
		}
	...
	public function get_field( $html, $formid, $form ) {
		...
		if ( empty( $form->inline ) ) { $html .= '<label for="mailster-_coolcaptcha-' . $formid . '">' . $label . '</label>';
		}
		$img = sprintf('<div><img title="' . __( 'click to reload', 'mailster-coolcaptcha' ) . '" onclick="var s=this.src;this.src=s.replace(/nocache=\d+/, \'nocache=\'+(+new Date()))" src="%s" style="cursor:pointer;width:%dpx;height:%dpx"></div>', add_query_arg(array(
			'action' => 'mailster_coolcaptcha_img',
			'nocache' => time(),
			'formid' => $formid,
